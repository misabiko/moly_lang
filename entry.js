(() => {
  ...
  // lezer.js
  var spec_Identifier = { __proto__: null, let: 36, true: 42, false: 44, return: 46, fn: 50, while: 58 };
  ...
  // lezerGrammar.ts
  var molyLanguage = LRLanguage.define({
  ...
  var exampleCompletion = molyLanguage.data.of({
  ...
  function eval_wasm(input) {
    try {
      const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
      const ptr0 = passStringToWasm0(input, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len0 = WASM_VECTOR_LEN;
      wasm.eval_wasm(retptr, ptr0, len0);
  ...
  async function init(input) {
    if (typeof input === "undefined") {
      input = new URL("moly_bg.wasm", import_meta.url);
    }
  ...
  async function runCode() {
    await moly_default("./moly_bg.wasm");
    ...
    const display = new Uint8Array(1e4);
    const output = [];
    ...
    display.set(new Uint8Array(memory.buffer, 0, 1e4));
  ...
  var scaleImageData = (imageData, scale, ctx) => {
